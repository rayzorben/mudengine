...
const ofMax = (percent: number, max: string): string => t('cards.party.ofMax', { percent, max });
...
describe('the number beside a party bar', () => {
  ...
  it('says what the percentage is of, once the maximum is known', () => {
    expect(meterValue(0.3, 4434)).toBe(ofMax(30, (4434).toLocaleString()));
    expect(meterValue(0.3, 62)).toBe(ofMax(30, '62'));
  });
  ...
  it('separates thousands the way the rest of the chrome does', () => {
    expect(meterValue(0.5, 12000)).toBe(ofMax(50, (12000).toLocaleString()));
  });

  it('rounds the way the bar did before', () => {
    expect(meterValue(0.625, 80)).toBe(ofMax(63, '80'));
  });
});

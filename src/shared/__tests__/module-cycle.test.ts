 ...
 *
 * `tally` is imported **first**, deliberately, and nothing else may be added
 * above it: it is the one module `character.ts` still takes a value from to
 * build `EMPTY_CHARACTER`, so it is the entry a cycle would break through.
 *
 * It was `players` until the registry left the character (todo 730), and that
 * is the order that used to be broken: `character.ts` needed `NO_PLAYERS` to
 * build `EMPTY_CHARACTER` and `players.ts` needed `ALIGNMENTS` to parse a
 * record, two value imports pointing at each other. Entered through `players`,
 * `character` evaluated while `players` was still in flight and captured
 * `NO_PLAYERS` as `undefined` — so `EMPTY_CHARACTER.players` was `undefined`
 * rather than `{}`, permanently, for every consumer in that graph.
 *
 ...
 */
import { NO_TALLY } from '../tally';
import { describe, expect, it } from 'vitest';

import { EMPTY_CHARACTER, NO_COMBAT, NO_PARTY } from '../character';

describe('the shared state constants, entered through tally', () => {
  it('has the totals on the empty character rather than an undefined', () => {
    expect(EMPTY_CHARACTER.tally).toBe(NO_TALLY);
  });
